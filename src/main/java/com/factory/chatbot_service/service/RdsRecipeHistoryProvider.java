...
@Service
public class RdsRecipeHistoryProvider implements RecipeHistoryProvider {

    private final NamedParameterJdbcTemplate jdbcTemplate;

    public RdsRecipeHistoryProvider(NamedParameterJdbcTemplate jdbcTemplate) {
    ...
    public List<RecipeHistoryCase> findRelevantHistories(
        ...
        List<RecipeHistoryCase> histories = jdbcTemplate.query(HISTORY_QUERY, params, this::mapHistoryCase);
    ...
    private List<RecipeParameterValue> findParameters(Long equipmentRecipeId) {
        ...
        return jdbcTemplate.query(HISTORY_PARAMETERS_QUERY, params, this::mapParameter);
    }
    ...
    private static final String HISTORY_QUERY = """
            ...
            WHERE CAST(er.equipment_id AS CHAR) = :equipmentId
                AND CAST(mr.process_id AS CHAR) = :processId
                AND CAST(mr.product_id AS CHAR) = :productId
            GROUP BY
                er.equipment_rec_id,
                er.equipment_id,
                mr.process_id,
                mr.product_id
            ORDER BY defect_rate ASC, er.equipment_rec_id DESC
            LIMIT 10
            """;

    private static final String HISTORY_PARAMETERS_QUERY = """
            SELECT
                erd.param,
                erd.min AS min_value,
                erd.max AS max_value
            FROM equipment_recipe_detail erd
            WHERE erd.equipment_rec_id = :equipmentRecipeId
            ORDER BY erd.param
            """;
}